...
L1 = dict(
    ...
    zones=[
        ...
        # Freight shaft — the blueprint labels it "freight elev.", so it is
        # toned back-of-house rather than as a passenger elevator.
        dict(role="boh", label="Freight", sub="", label_at=(120, 416),
             size="xs", label_anchor="middle", pts=rect(95, 388, 145, 432)),
    ...
    pins=[
        ...
        # Both east-side door clusters (page-bottom = east): the blueprint's
        # 67" clear-width doors, same positions the event map pins.
        dict(icon="entrance", role="event", at=(412, 672), label="Entrance",
             note="Bay St. & TD Concourse", side="left", show_label=True),
        dict(icon="entrance", role="event", at=(200, 690)),
    ],
    callouts=[],
...
L2 = dict(
    ...
    zones=[
        ...
        # Freight shaft — labelled "Freight Elev." on the blueprint; crew only.
        dict(role="boh", label="Freight", sub="", label_at=(122, 422),
             size="xs", label_anchor="middle", pts=rect(98, 392, 146, 440)),
    ...
    pins=[
        ...
        dict(icon="stairs", role="stair", at=(160, 658), label="Grand Staircase",
    ...
    callouts=[
        dict(at=(306, 500), w=215, title="No washrooms on this level",
             body="Nearest washrooms are one level down in the Lobby, or one "
                  "level up in the Gallery. Stairs sit in all four corners; "
                  "elevators are on the south side and in the north-east "
                  "corner.", warn=True),
    ],
    connections="The Grand Staircase lands in the south-east corner of the "
                "floor. Washrooms are on Level 1 and Level 3 only.",
)
...
L3 = dict(
    ...
    zones=[
        ...
        # Freight shaft — labelled "freight elev." on the blueprint; crew only.
        dict(role="boh", label="Freight", sub="", label_at=(466, 450),
             size="xs", label_anchor="middle", pts=rect(438, 410, 495, 478)),
    ...
    pins=[
        ...
        dict(icon="washroom", role="washroom", at=(368, 556)),
