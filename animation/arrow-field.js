...
const CONFIG = {
  width: 6878,
  height: 1080,
  duration: 180, // seconds, one full loop
  background: '#8A25C9',
  gradientFrom: '#b659f0', // lower-left end of each arrow
 ...
 * perpendicular to the travel direction once the unit box is stretched back to
 * the arrow's 422.98 x 600 bbox. Solved so the gradient starts at the box corner
 * furthest down-left along the axis and ends at the corner furthest up-right.
 */
function gradientEndpoints() {
  const dx = RUN;
  const dy = -RISE;
  // Band normal in bbox units must satisfy (gx / ARROW.w) : (gy / ARROW.h) = dx : dy.
  const k = (ARROW.h / ARROW.w) * (dy / dx); // gy = k * gx
...
function buildTile(layer, index, cfg) {
  ...
  for (const p of poissonTorus(layer.tileW, tileH, layer.count, rng)) {
    ...
    const w = ARROW.w * scale;
    const h = ARROW.h * scale;
    const x0 = p.x - w / 2;
    const y0 = p.y - h / 2;
    ...
    for (let i = -1; i <= 1; i++) {
      for (let j = -1; j <= 1; j++) {
        ...
        const y = y0 + j * tileH;
        if (x > layer.tileW || x + w < 0 || y > tileH || y + h < 0) continue;
        parts.push(
          `<use href="#a" transform="translate(${x.toFixed(2)} ${y.toFixed(2)}) ` +
  ...
  const svg =
    ...
    '</linearGradient>' +
    `<polygon id="a" points="${ARROW.points}" fill="url(#g)"/>` +
    '</defs>' +
    parts.join('') +
    '</svg>';
...
function mountArrowField(stage, cfg = CONFIG) {
  ...
  stage.style.background = cfg.background;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { CONFIG, ARROW, RISE, RUN, mulberry32, gradientEndpoints, buildTile, mountArrowField };
}
